...
switch ($option) {
    ...
    case 'save':
        ...
        if ($id_product == '') {
            ...
            if (empty($consult)) {
                ...
                if ($result) {
                    $res = array('tipo' => 'success', 'mensaje' => 'PRODUCTO REGISTRADO CON EXITO');
                } else {
                    $res = array('tipo' => 'error', 'mensaje' => 'ERROR AL AGREGAR');
                }
            } else {
                $res = array('tipo' => 'error', 'mensaje' => 'EL BARCODE YA EXISTE INTENTALO CON OTRO');
            }
        } else {
            $result = $productos->updateProduct($barcode, $nombre, $precio, $stock, $id_product);
            if ($result) {
                $res = array('tipo' => 'success', 'mensaje' => 'PRODUCTO MODIFICADO CON EXITO');
            } else {
    ...
    case 'delete':
        ...
        if ($data) {
            $res = array('tipo' => 'success', 'mensaje' => 'PRODUCTO ELIMINADO CON EXITO');
        } else {
